...
:revnumber: 0.2.0-dev.4+20190407
...
pronunciation and are all sounded. The "`y`" (initial and medial) are
pronounced as in "`yes`", "`ey`" (final) as in "`they`", and "`eu`" as
"`éh-oo`".
...
pleasure: *plage*, *giraffe*. Elsewhere "`g`" is hard as in "`go`".
...
As [t], except when followed by "`ia`", "`io`", "`iu`", or "`ie`" and
not preceded by an "`s`".  Thus the t in *nation* is pronounced [ts]
but in *bastion* as [t].
